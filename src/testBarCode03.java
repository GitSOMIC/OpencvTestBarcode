...
//        cv2.waitKey(0)
        ...
        System.out.println("  "+ srcImag.width()+ "  "+srcImag.height()+ "   "+height+ "   "+width);

        Point[] pt1 = new Point[4];//输出图像焦点
        pt1[0] = new Point(0, 0);
        pt1[1] = new Point(srcImag.width(), 0);
        pt1[2] = new Point(srcImag.width(), srcImag.height());
        pt1[3] = new Point(0, srcImag.height());

        Point[] pt2 = new Point[4];//输入图像焦点
        pt2[0] = new Point(0,0);
...
//        Mat mat = Imgproc.getAffineTransform(mop1, mop2);//计算仿射变换2*4矩阵

        Mat perspectiveTransform = Imgproc.getPerspectiveTransform(mop1, mop2);
        Mat cut = new Mat();
        Imgproc.warpPerspective(srcImag,cut,perspectiveTransform,new Size(srcImag.width(),srcImag.height() ));
        HighGui.imshow("cut",cut);
        HighGui.waitKey();
//#二值化 筛掉一些错误
