...
//                img=cv2.imread(r'XX.png',1) #读取图片
        Mat srcImag = Imgcodecs.imread("imag/oneCode/06code128/code12803.png");
...
//        	Imgproc.THRESH_BINARY：当像素值超过阈值thresh时取maxval，否则取0；
        Imgproc.threshold(blurImage, thresholdImage, 150, 255, Imgproc.THRESH_BINARY);
...
//        int v1 = (int) Math.ceil(by * 100 * 5) ;
        int v = (int) (by * 100 * 5);  //by是小数，int强转执行顺序要注意
