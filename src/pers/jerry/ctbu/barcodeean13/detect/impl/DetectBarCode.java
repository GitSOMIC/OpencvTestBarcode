package pers.jerry.ctbu.barcodeean13.detect.impl;

import org.opencv.core.*;
import org.opencv.highgui.HighGui;
import org.opencv.imgcodecs.Imgcodecs;
import org.opencv.imgproc.Imgproc;
import pers.jerry.ctbu.barcodeean13.detect.Detect;
 ...
 * @author Jerry
 * @version 1.0
 * @Description TODO 定位一维码位置
 */
public class DetectBarCode implements Detect {
    static {
        System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
    }

    private static Rect pyke;

    public static Rect getPyke() {
        return pyke;
    }
...
//  将图片变为灰度图片
        ...
        Imgcodecs.imwrite("1.jpg", grayImage);
        HighGui.waitKey();

//使用Sobel算子,求得水平和垂直方向灰度图像的梯度差 - 对X和Y方向求"微分"
//
        Mat gradX = new Mat();
        Mat gradY = new Mat();
...
//      二值化
        ...
        Imgcodecs.imwrite("4二值化图像.jpg", thresholdImage);
        HighGui.waitKey();

//      闭运算 填充条码空隙 （参数需要调整）
        //获取Kernel （10，5）
        Mat kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(30, 10));  //自己改成5
        ...
        Imgcodecs.imwrite("5.jpg", morphImage);
        HighGui.waitKey();
//      4次腐蚀 再4次膨胀：消除小斑点
//                closed2 = cv2.erode(closed1, None, iterations = 4)
        ...
        Imgcodecs.imwrite("6.jpg", dilatingImage);
        HighGui.waitKey();
//      开运算
...
//        HighGui.waitKey();
        //获取Kernel （10，10）
        Mat kernel3 = Mat.ones(new Size(30, 10), CvType.CV_8UC1);
        ...
        Imgcodecs.imwrite("7开运算图像.jpg", morphImage2);
        HighGui.waitKey();
//      腐蚀
//        （5，5）
        ...
        Imgcodecs.imwrite("8.jpg", morphImage2);
        HighGui.waitKey();
//        Imgcodecs.imwrite("腐蚀图像*2.jpg", morphImage2);
//      Canny找出边界

        Imgproc.Canny(morphImage2, morphImage2, 60, 200);
        HighGui.imshow("Canny(60,200)", morphImage2);
        Imgcodecs.imwrite("9.jpg", morphImage2);
        HighGui.waitKey();
...
//            boundingRect()获得最小外接矩形的矩形边框
        ...
        Imgcodecs.imwrite("10frame.jpg", srcImagCloe);
        HighGui.waitKey(0);
...
//                result.get(3, 1)[0]);
        int widthd = (int) max(result.get(0, 0)[0], result.get(1, 0)[0], result.get(2, 0)[0],
                result.get(3, 0)[0]) - x0;  // 减 x0

        System.out.println(result.get(0, 1)[0] + "  " + result.get(1, 1)[0] + "  " + result.get(2, 1));
        int heightd = (int) max(result.get(0, 1)[0], result.get(1, 1)[0], result.get(2, 1)[0],
                result.get(3, 1)[0]) - y0;  // 减 y0
...
//        double srcImagArea = contourArea(srcImag);  //源图片区域面积  srcImage是图，contourArea是计算轮廓面积
        ...
        System.out.println("一维码区域宽度占比" + wy);
//        int v1 = (int) Math.ceil(by * 100 * 5) ;
//        int v = (int) (by * 100 * wy * 50);  //by是小数，int强转执行顺序要注意
        int v = (int) (1 / wy * 10);  //by是小数，int强转执行顺序要注意
