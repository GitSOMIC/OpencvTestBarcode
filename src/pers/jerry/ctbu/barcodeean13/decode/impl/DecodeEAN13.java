package pers.jerry.ctbu.barcodeean13.decode.impl;
...
import pers.jerry.ctbu.barcodeean13.decode.Decode;
import pers.jerry.ctbu.barcodeean13.pojo.Code;
import pers.jerry.ctbu.barcodeean13.pojo.impl.LeftCode;
import pers.jerry.ctbu.barcodeean13.pojo.impl.RightCode;
 ...
 * @author Jerry
 * @version 1.0
 * @Description TODO 对EAN-13码进行解码
 */
public class DecodeEAN13 implements Decode {
    static {
        System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
    }

    private static String ean13True = "";
...
//        StringBuilder codes = new StringBuilder();
     ...
     * @param line
     */
    public String decodeLine(List line) {
        ...
        if (verify) {
            ean13True = ean13;
            System.out.println("条码值:" + ean13True + " 有效性:" + verify);
            isSuccess = true;
            return "Success";
        }
        return "Failing";
...
//            System.out.println(decode);
    ...
    public void decode(Mat detect) {
         ...
         */
        //将获取到的detect 截取好的一维码图片 放入 getMat中处理
        Mat threshNot = getMat(detect);
        ...
        for (int i = height - 1; i > 0; i--) {
            try {
                ...
                String success = decodeLine(line);
                if ("Success".equals(success)) {
                    System.out.println("读取到:" + i + "行解码成功");
                    break;
                }
            } catch (Exception e) {
...
//        HighGui.destroyAllWindows();
     ...
     * @return
     */
    public Mat getMat(Mat imag) {
        //灰度化，如果detect之后就已经灰度化过了，再次灰度化会报错
        /*
         ...
         */
        Mat detGray = new Mat();
        Imgproc.cvtColor(imag, detGray, Imgproc.COLOR_RGB2GRAY);
        HighGui.imshow("灰度图片", detGray);
        HighGui.waitKey();
        //二值化 阈值化
        Mat thresh = new Mat();
        Imgproc.threshold(detGray, thresh, 120, 255, Imgproc.THRESH_OTSU);
//        Imgproc.threshold(detGray, thresh, 200, 255, Imgproc.THRESH_BINARY + Imgproc.THRESH_OTSU);
//        Imgproc.threshold(gray, thresh, 125, 255, Imgproc.THRESH_BINARY | Imgproc.THRESH_OTSU);
        HighGui.imshow("thresh", thresh);
        HighGui.waitKey();
        //取反
...
//        }
    ...
    public boolean verify(String ean13) {
        ...
        //取出字符串最后一位
//        int units_digit = Integer.parseInt (String.valueOf(ean13.charAt(ean13.length() - 1)));
        int checksum = 10 - weighted_sum % 10;
        if (checksum == 10) {
            checksum = 0;
        }
...
//            weightedSum = weightedSum + weight[i] * Integer.parseInt(String.valueOf(ean13.charAt(i)));
//        }
//        String weightedSumString = weightedSum + "";
//        int checksum = 0;
//        //取出字符串最后一位
//        int units_digit = Integer.parseInt(String.valueOf(weightedSumString.charAt(weightedSumString.length() - 1)));
